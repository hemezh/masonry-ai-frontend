...
export function TableList({ 
  ...
  const getDaysRemaining = (archivedAt: string | null) => {
    if (!archivedAt) return null;
    const archiveDate = new Date(archivedAt);
    const deleteDate = new Date(archiveDate.getTime() + (30 * 24 * 60 * 60 * 1000)); // 30 days in milliseconds
    const now = new Date();
  ...
  return (
    <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
      {tables.map((table) => {
        const daysRemaining = table.archived ? getDaysRemaining(table.archived_at) : null;
        ...
        return (
          ...
          >
            ...
            <div className="text-xs text-muted-foreground dark:text-slate-400 border-t border-border/40 dark:border-slate-600 pt-2 mt-auto relative z-10 pointer-events-none">
              {table.archived && daysRemaining !== null ? (
                <div className="flex flex-col gap-1">
                  <span className={`font-medium ${daysRemaining <= 7 ? 'text-destructive' : ''}`}>
                    {daysRemaining} days until deletion
                  </span>
                  <span>Archived on {formatDate(table.archived_at!)}</span>
                </div>
              ) : (
