...
const MARKDOWN_STYLES = {
    ...
    code: "[&>pre]:my-2 [&>pre]:p-0 [&>pre]:rounded-md [&>pre]:bg-transparent [&>p>code]:bg-muted [&>p>code]:px-1.5 [&>p>code]:py-0.5 [&>p>code]:rounded-md [&>p>code]:before:hidden [&>p>code]:after:hidden [&>p>code]:text-sm",
...
const StepContent = ({ step, showDetails, contentRef }: StepContentProps) => {
    ...
    return (
        <div className="w-full max-w-full overflow-x-auto rounded-md bg-muted text-muted-foreground">
...
const StepStatus = ({ status }: { status: Step['status'] }) => {
    const statusIcons = {
        ...
        skipped: <ArrowRightIcon className="h-5 w-5 text-muted-foreground" />,
        pending: <div className="h-5 w-5 rounded-full border-2 border-muted" />
    };
...
const ContentBlock = ({ block, steps }: { block: ContentBlockType; steps: Record<string, Step> }) => {
    ...
    if (block.type === 'step' && block.stepId) {
        ...
        return (
            <div className="flex flex-col my-2 py-1 border bg-background border-border rounded-md text-sm">
                <div
                    className="flex items-center gap-2 px-2 py-1 cursor-pointer"
                    onClick={() => setShowDetails(!showDetails)}
                >
                    <StepStatus status={step.status} />
                    <span className="flex-1 text-foreground">{step.content}</span>
                    {step.details && (
                        <ChevronDownIcon 
                            className={cn(
...
export function ChatMessage({ message }: { message: ChatMessageType }) {
    return (
        <div className={cn(
            ...
            message.role === "assistant" && "bg-muted/50 rounded-lg"
        )}>
            {message.role === "user" && (
